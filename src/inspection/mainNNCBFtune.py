...
# 17th JAn, running with higher dv weight. Note that RNN will need adaption to this too. 
import sys
...
# ----------------------------------------------------------------------
def make_env(rank, seed, dt, enable_param_randomisation, enableNoise, enableCBFtunning, dvWeight):
    def _init_():
        env = InspectionEnv(dt=dt,
...
def run_jais_training():
    ...
    # we need to try higher dv weight
    dvWeight= 10.0

    # ---------------- env / timing ----------------
    totalEpisodes   = int(1e5)   # just used to scale total_timesteps
    if is_debug_mode():
        num_env = 1
    else:
        num_env = 64
    ...
    dt = 10
    stoch_env = InspectionEnv(dt=dt,  enable_param_randomisation=enable_param_randomisation,enableNoise = enableNoise, enableCBFtunning = enableCBFtunning, dvWeight=dvWeight )
...
#withRTA_
    ...
    deter_env = InspectionEnv(
        ...
        enableNoise = enableNoise, 
        enableCBFtunning = enableCBFtunning, dvWeight=dvWeight)
    #     eval_mode=True,
    ...
    if num_env > 1:
        train_env = SubprocVecEnv([make_env(i, seed, dt, enable_param_randomisation, enableNoise, enableCBFtunning, dvWeight) for i in range(num_env)])
    else:
        train_env = stoch_env
    ...
    if modelLoad:
        ...
        deter_env = InspectionEnv(dt, enable_param_randomisation=enable_param_randomisation, enableNoise=enableNoise, enableCBFtunning=enableCBFtunning, dvWeight=dvWeight )
        model = PPO.load(
            os.path.join(eval_log_dir, "best_model.zip"),
            env=deter_env,
            tensorboard_log=tensorboard_log_dir_source,
    ...
    n_samples = 10  # evaluate across the full bank
