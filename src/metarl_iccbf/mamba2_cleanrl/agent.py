"""Mamba2-based actor-critic for CleanRL-style PPO.

Pure ``nn.Module`` with native Mamba2 state shapes — no SB3 recurrent
internals, no LSTM packing hacks, no ``_FakeLSTMAttrs``.

Two code paths, both with proper episode-boundary handling:
- ``step()``  — single-timestep ``mamba.step()`` for rollout collection
- ``evaluate_actions()``  — ``mamba.forward()`` parallel scan for training

Architecture modes (controlled by ``morl_arch``):
- ``"concat"`` (default): single ``proj_actor/proj_critic`` Linear fed with
  ``concat(obs, w)``; existing behavior, no API change.
- ``"multi_body"``: K separate Linear+ReLU projections per head, weighted by
  the preference vector w before the Mamba2 encoder (MOPPO paper eq. 10).
  obs passed to the agent is raw (no w appended); w is a separate argument.
"""
...
class Mamba2ActorCritic(nn.Module):
    ...
    obs_dim : int
        ...
        is the raw obs dim (w is passed separately).
    act_dim : int
    ...
    morl_arch : str
        ``"concat"`` (default) or ``"multi_body"``.
    n_objectives : int
        Number of preference weight dimensions K.  Only used when
    ...
    def __init__(
        ...
        *,
        ...
        if morl_arch == "multi_body":
            ...
            # Register dummy attributes so save/load code that checks for
            # proj_actor/proj_critic doesn't crash when morl_arch != "concat".
    ...
    def step(
        ...
        w : (n_envs, K)  preference weights — required when
            ``morl_arch="multi_body"``, ignored otherwise.
    ...
    def evaluate_actions(
        ...
        w : (B, T, n_objectives)  preference weights per step (multi_body only)
        context_w : (B, K, n_objectives)  burn-in context weights (multi_body only)
    ...
    def get_value(
        ...
        ----------
        obs : (B, obs_dim)
        critic_states : (conv_state, ssm_state)
        episode_starts : (B,)
        w : (B, K)  preference weights — required when morl_arch="multi_body"
    ...
    def _multi_body_proj(
        ...
        Parameters
        ----------
        obs : (..., obs_dim)   any leading batch/time dims
        w   : (..., K)         preference weights (same leading dims as obs)
    ...
    def _scan(
        ...
        episode_starts : (B, L)
        proj : Linear projection for "concat" mode (mutually exclusive with body_list)
        ...
        if body_list is not None:
            projected = self._multi_body_proj(obs, w, body_list)  # (B, L, d_model)
        else:
