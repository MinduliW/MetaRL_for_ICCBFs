...
def sweep_pareto_front(
    ...
    *,
    ...
    n_obj = model.morl_n_objectives

    device = model.device
    agent = model.agent

    t_vals  = np.linspace(weight_lo, weight_hi, n_weights, dtype=np.float32)
    W_COVERAGE = 0.6
    weights = np.stack([
        np.full(n_weights, W_COVERAGE, dtype=np.float32),
    ...
    for wi, w in enumerate(weights):
        ...
        for _ in range(n_episodes_per_weight):
            ...
            ep_r_vec = np.zeros(n_obj, dtype=np.float32)
            done = False
        ...
        if verbose:
            ...
            print(
                ...
                f"coverage={m[0]:.3f}  fuel={m[1]:.3f}  safety={m[2]:.3f}"
            )
