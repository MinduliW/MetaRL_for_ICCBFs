...
from inspectionEnv import InspectionEnv
import warnings
...
def run_jais_training():
    ...
    # ---------------- env / timing ----------------
    totalEpisodes   = int(8e5)   # just used to scale total_timesteps
    if is_debug_mode():
        num_env = 1
    else:
        num_env = 8
    ...
    batch_size      = 64
    n_steps         = int(batch_size * n_steps_factor*8/num_env)

#withRTA_
    training_name = (
        "OtherInspectionPPO_wuRL"
        + "_l" + str(layers)
        + "_n" + str(nodes)
    ...
    if trainON:
        ...
        model.save(os.path.join(eval_log_dir, "best_model.zip"))
        del model  # free memory

    # ---------------- load best model for plotting ----------------
    if modelLoad:
        print("Loading best model for plotting...")
        deter_env = InspectionEnv(dt)
        model = PPO.load(
            os.path.join(eval_log_dir, "best_model.zip"),
            env=deter_env,
            tensorboard_log=tensorboard_log_dir_source,
    ...
    n_samples = 100  # evaluate across the full bank
    ...
    def plot_all_constraints(plot_log_dir: str, env: JAISInspectionEnv, fft_stride: int = 50):
        """
